...
5. Additionally, this package provides `CallFuncPx` (where `x` corresponds to the number of arguments, `0~3`) for in-place invocation, suitable for external functions that do not require stack switching (But please note that if you cannot ensure that the stack space required by the external function is zero, using this method may cause memory corruption). On amd64, unlike `CallFuncG0Px`, the in-place variants do not normalize the stack pointer: Go only guarantees 8-byte alignment, so the callee must additionally tolerate an SP that is 8-byte but not 16-byte aligned (functions using aligned SSE/AVX stack accesses may fault). On arm64 the in-place variants do normalize the stack pointer to 16-byte alignment. Use `CallFuncG0Px` when the callee needs full ABI alignment everywhere.
