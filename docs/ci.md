...
- Matrix job for everything Go-only, with the toolchain selected via `actions/setup-go` (the values live in the workflow-level `GO_VERSION` / `GO_MIN_VERSION` env vars and are repeated as matrix literals because matrices cannot reference `env`). `GO_VERSION` is the `stable` alias, which always resolves to the latest Go release — CI follows new Go versions automatically instead of going stale on a pinned literal; only the minimum supported toolchain is pinned:
  ...
  - **macOS × stable** (arm64): build + test + `go vet`. The `asmcall/calltest` package is compiled with darwin-enabled build tags (`(linux || darwin) && (amd64 || arm64)`), so this leg compiles and vets the arm64 assembly path of `asmcall`, which the ubuntu legs never build. The trampoline *tests* stay Linux-only (`call_test.go` is gated `linux && (amd64 || arm64)`): on darwin/arm64 the non-G0 variants (plain CALL on the goroutine stack) were observed to hang in CI while the G0 variants pass — until root-caused, macOS provides compile/vet coverage only.
