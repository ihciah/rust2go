...
- Matrix job for everything Go-only, with the toolchain selected via `actions/setup-go` (the values live in the workflow-level `GO_VERSION` / `GO_MIN_VERSION` env vars and are repeated as matrix literals because matrices cannot reference `env`). `GO_VERSION` is the `stable` alias, which always resolves to the latest Go release — CI follows new Go versions automatically instead of going stale on a pinned literal; only the minimum supported toolchain is pinned:
  ...
  - **ubuntu-24.04-arm × stable** (linux/arm64): build + test only, over the same packages. The amd64 and arm64 asmcall trampolines differ in how return addresses are handled (stack vs LR), so the trampoline tests need real coverage on both ISAs — an arm64-only bug (LR clobbered by BLR) previously shipped while linux/amd64 stayed green.
  - **macOS × stable** (arm64): build + test + `go vet`. This leg exercises the arm64 assembly path of `asmcall` (trampoline tests included, `call_test.go` is gated `(linux || darwin || windows) && (amd64 || arm64)`) on darwin, whose toolchain and runtime differ from linux/arm64.
  - **windows-latest × stable** (amd64): asmcall-only (`go test ./asmcall/...`). `amd64.s` has a Win64 calling-convention path (arguments in CX/DX/R8, selected via `GOOS_windows`) that no other leg exercises; the runner image's MSYS2 mingw-w64 gcc covers the cgo-built `calltest` callees. mem-ring is unix-only and `test/go` depends on it, so neither runs on this leg.
