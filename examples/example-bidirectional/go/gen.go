...
type DemoCall interface {
	demo_oneway(user *DemoUser)
	demo_call(user *DemoUser) DemoResponse
}

//export CDemoCall_demo_oneway
func CDemoCall_demo_oneway(user C.DemoUserRef) {
	_new_user := newDemoUser(user)
	DemoCallImpl.demo_oneway(&_new_user)
}

//export CDemoCall_demo_call
func CDemoCall_demo_call(user C.DemoUserRef, slot *C.void, cb *C.void) {
	_new_user := newDemoUser(user)
	resp := DemoCallImpl.demo_call(&_new_user)
...
func (G2RCallImpl) demo_log(name *string, age *uint8) {
	_internal_params := [2]unsafe.Pointer{}
	name_ref, name_buffer := cvt_ref(cntString, refString)(name)
	_internal_params[0] = unsafe.Pointer(&name_ref)
	age_ref, age_buffer := cvt_ref(cntC_uint8_t, refC_uint8_t)(age)
	_internal_params[1] = unsafe.Pointer(&age_ref)
	asmcall.CallFuncG0P1(unsafe.Pointer(C.c_G2RCall_demo_log), unsafe.Pointer(&_internal_params))
...
func (G2RCallImpl) demo_convert_name(user *DemoUser) string {
	...
	asmcall.CallFuncG0P2(unsafe.Pointer(C.c_G2RCall_demo_convert_name), unsafe.Pointer(&_internal_slot), unsafe.Pointer(&_internal_params))
