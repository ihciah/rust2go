...
One side creates a ring with `Queue::new(size)`, which allocates the buffer and the atomics and returns a `QueueMeta` next to the queue. `QueueMeta` is a plain `#[repr(C)]` struct carrying the raw addresses (as `usize`/`uintptr`) and the peer ends of two socketpairs; passing it to the other language lets that side reconstruct a view of the same ring (`Queue::new_from_meta` on Rust, `NewQueue` on Go). The creator owns the memory and frees it on drop; the side built from the meta owns only the fds it received. Peers must stop using a queue before its memory owner is dropped: dropping the creator while the other side still pushes or pops is a use-after-free. (Materializing pointers from raw integers trips `go vet`'s unsafeptr analyzer, which is why CI runs vet with `-unsafeptr=false`.)
