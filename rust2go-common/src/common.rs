...
impl ParamType {
    ...
    /// Render the ref type in associated-type form (`<T as ToRef>::Ref`).
    /// Unlike [`ParamType::to_rust_ref`], this also works when the type is a
    /// type alias, so it is used by the proc-macro side which cannot resolve
    /// aliases on its own.
    pub fn to_rust_ref_assoc(&self, prefix: Option<&TokenStream>) -> TokenStream {
        match &self.inner {
            ParamTypeInner::Primitive(name) => quote!(#name),
            ParamTypeInner::Custom(name) => {
                quote!(<#prefix #name as ::rust2go::ToRef>::Ref)
            }
...
/// Collect `type A = B;` aliases and expand them inside struct fields and
/// trait signatures, so that all downstream code only sees concrete types.
fn expand_type_aliases(file: &mut File) {
    let mut aliases: HashMap<Ident, Type> = HashMap::new();
    for item in file.items.iter() {
        if let Item::Type(item_type) = item {
            // Generic aliases like `type A<T> = Vec<T>;` are not supported.
            if item_type.generics.params.is_empty() {
                aliases.insert(item_type.ident.clone(), (*item_type.ty).clone());
            }
        }
    }
    if aliases.is_empty() {
    ...
    // Resolve alias chains (`type A = B; type B = i64;`) and detect cycles.
    for key in aliases.keys().cloned().collect::<Vec<_>>() {
        let mut chain = vec![key.clone()];
        let mut resolved = aliases.get(&key).unwrap().clone();
        while let Some(ident) = bare_path_ident(&resolved) {
            ...
            if chain.contains(ident) {
                let chain_str = chain
                    ...
                    .collect::<Vec<_>>()
                    .join(" -> ");
                panic!("cyclic type alias detected: {chain_str} -> {ident}");
            }
        ...
        match item {
            Item::Struct(item_struct) => {
                for field in item_struct.fields.iter_mut() {
                    expand_type_aliases_in_type(&mut field.ty, &aliases);
                }
            }
                    ...
                    if let syn::TraitItem::Fn(f) = trait_item {
                        for arg in f.sig.inputs.iter_mut() {
                            if let syn::FnArg::Typed(pat_type) = arg {
                                expand_type_aliases_in_type(&mut pat_type.ty, &aliases);
                            }
                        }
                        if let syn::ReturnType::Type(_, ty) = &mut f.sig.output {
                            expand_type_aliases_in_type(ty, &aliases);
                        }
                    }
                }
            }
...
/// If `ty` is a bare single-segment path without generic arguments
/// (e.g. `Amount`), return its ident.
fn bare_path_ident(ty: &Type) -> Option<&Ident> {
...
fn expand_type_aliases_in_type(ty: &mut Type, aliases: &HashMap<Ident, Type>) {
    if let Some(ident) = bare_path_ident(ty) {
        if let Some(target) = aliases.get(ident) {
            *ty = target.clone();
            // The target itself may contain aliases in its generic arguments.
            expand_type_aliases_in_type(ty, aliases);
            return;
        }
    }
        ...
        Type::Path(type_path) => {
            for seg in type_path.path.segments.iter_mut() {
                if let syn::PathArguments::AngleBracketed(args) = &mut seg.arguments {
                    expand_type_aliases_in_args(args, aliases);
                }
            }
        }
        Type::Reference(type_ref) => expand_type_aliases_in_type(&mut type_ref.elem, aliases),
        Type::Tuple(type_tuple) => {
            for elem in type_tuple.elems.iter_mut() {
                expand_type_aliases_in_type(elem, aliases);
            }
        }
...
fn expand_type_aliases_in_args(
    args: &mut syn::AngleBracketedGenericArguments,
    aliases: &HashMap<Ident, Type>,
) {
    for arg in args.args.iter_mut() {
        match arg {
            syn::GenericArgument::Type(ty) => expand_type_aliases_in_type(ty, aliases),
            syn::GenericArgument::AssocType(assoc) => {
                expand_type_aliases_in_type(&mut assoc.ty, aliases)
            }
            _ => {}
        }
    }
...
mod tests {
    ...
    fn type_alias_expansion() {
        ...
        #[rust2go::r2g]
        pub trait DemoCall {
            fn demo_check(user: &DemoUser, amount: Amount) -> bool;
            fn demo_list(amounts: Amounts) -> Amount;
        }
        "#;
        let raw_file = super::RawRsFile::new(raw);

        // Struct field aliases are resolved when computing levels.
        let levels = raw_file.convert_structs_levels().unwrap();
        let demo_user = proc_macro2::Ident::new("DemoUser", proc_macro2::Span::call_site());
        assert_eq!(levels.get(&demo_user), Some(&1));

        // The generated ref structs use concrete types instead of `{Alias}Ref`.
        let (_mapping, ref_structs) = raw_file.convert_structs_to_ref().unwrap();
        let ref_structs = ref_structs.to_string();
        assert!(!ref_structs.contains("AmountRef"));
        assert!(ref_structs.contains("i64"));

        // Trait params and return types with aliases generate fine.
        let traits = raw_file.convert_r2g_trait().unwrap();
        assert_eq!(traits.len(), 1);
        let go_interface = traits[0].generate_go_interface();
        assert!(!go_interface.contains("Amount"));
        assert!(go_interface.contains("int64"));
        let go_structs = raw_file.convert_structs_to_go(&levels, false).unwrap();
        assert!(!go_structs.contains("AmountRef"));
    }

    #[test]
    #[should_panic(expected = "cyclic type alias")]
    fn cyclic_type_alias() {
        let raw = r#"
        pub type A = B;
        pub type B = A;
        "#;
        let _ = super::RawRsFile::new(raw);
    }
}
