...
/// Arguments for Go code generation.
///
/// This mirrors the `rust2go-cli` flags, but is a plain data struct so that
/// library users (e.g. `rust2go`'s build helper) do not pull in `clap`.
...
pub fn generate(args: &GenArgs) {
    ...
    let use_cgocall = r2g_any!(|f| f.mem_call_id().is_none() && f.cgo_callback() && f.ret().is_some())
...
fn get_package_name(args: &GenArgs) -> &str {
