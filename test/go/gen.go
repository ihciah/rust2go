...
func CTestCall_add_friends(req C.FriendsListRequestRef, slot *C.void, cb *C.void) {
	fmt.Println("CCC")
	// convert slot pointer to usize to prevent pointer checking when grow stack
	ptr_num := uintptr(unsafe.Pointer(slot))
	go func() {
		fmt.Println("BBB")
		time.Sleep(time.Duration(1)*time.Second)
		resp := TestCallImpl.add_friends(newFriendsListRequest(req))
		resp_ref, buffer := cvt_ref(cntFriendsListResponse, refFriendsListResponse)(&resp)
		C.TestCall_add_friends_cb(unsafe.Pointer(cb), resp_ref, unsafe.Pointer(ptr_num))
		runtime.KeepAlive(resp)
		runtime.KeepAlive(buffer)
	}()
}
...
func CTestCall_delete_friends(req C.FriendsListRequestRef, slot *C.void, cb *C.void) {
	go func() {
		resp := TestCallImpl.delete_friends(newFriendsListRequest(req))
		resp_ref, buffer := cvt_ref(cntFriendsListResponse, refFriendsListResponse)(&resp)
...
func CTestCall_pm_friend(req C.PMFriendRequestRef, slot *C.void, cb *C.void) {
	go func() {
		resp := TestCallImpl.pm_friend(newPMFriendRequest(req))
